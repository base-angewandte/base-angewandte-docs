...
- The **config** directory contains our common base setup for linting, pre-commit hooks, and make commands. It is
  ...
  - _.cz.toml_, _.hadolint.yaml_, and _uv.toml_ will usually be linked to, as they contain the linting configuration
    to adhere to our [](./dev_guide.md#coding-conventions) and [](./using_git.md#commit-message-guidelines). No specific
    change on a repo basis should be needed.
  - _.pre-commit-config.yaml_ and _.ruff.toml_ will often be copied rather than be linked, particularly the
    ruff config, as most projects use different Python versions, and hence will need to set different
    target versions.
